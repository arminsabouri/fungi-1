...
use crate::intent::{IntentId, IntentWithPayoffCurve};
...
pub struct Queue(HashMap<IntentId, IntentWithPayoffCurve>);

impl Queue {
    /// Take intents into a queue, giving each one an id.
    pub fn new(intents: impl IntoIterator<Item = IntentWithPayoffCurve>) -> Self {
        Queue(
            intents
                .into_iter()
                .enumerate()
    ...
    pub fn get(&self, id: IntentId) -> Option<&IntentWithPayoffCurve> {
        self.0.get(&id)
    }

    /// Every intent, in no particular order.
    pub fn iter(&self) -> impl Iterator<Item = &IntentWithPayoffCurve> {
        self.0.values()
    }
}
...
mod tests {
    ...
    fn intent(start: Instant, sats: u64) -> IntentWithPayoffCurve {
        IntentWithPayoffCurve::new(
            Intent::OutputCreation(FixedPaymentInstructions {
